...
def start_chat(user_id: str):
    ...
    save_chat(user_id, chat_id, chat_name="New Chat", message=None)
    return {"chat_id": chat_id, "chat_name": "New Chat"}
...
def delete_files(user_id: str, chat_id: str):
    ...
    return {"message": "Files and vectors deleted"}

@router.post("/generate_name")
def generate_chat_name(request: GenerateNameRequest):
    model = get_llm_model()
    chat_name = get_chat_name(request.first_message, request.answer_text, model)
    update_chat_name(request.user_id, request.chat_id, chat_name)
    return {"chat_name": chat_name}

@router.post("/useful_history")
def useful_history(question: str, user_id: str, chat_id: str):
    model = get_llm_model()
    history = get_user_history(user_id, chat_id)
    useful = get_usefull_chat_history(history, question, model)
    return {"useful_history": useful}